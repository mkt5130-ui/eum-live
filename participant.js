import {sb,deviceId,esc,loadAll,keywordCounts,cloudHTML,subscribe,formatTime} from './common.js';
...
document.querySelectorAll('.like').forEach(b=>b.onclick=()=>like(b.dataset.id,b))}
async function like(qid,btn){btn.disabled=true;const {error}=await sb.from('eum_question_likes').insert({question_id:qid,device_id:did});if(error){if(String(error.code)==='23505'){btn.textContent='♥ 이미 공감했어요';btn.classList.add('done')}else alert('공감 처리 중 오류가 발생했습니다.')}else{btn.textContent='♥ 공감 완료';btn.classList.add('done')}setTimeout(()=>{btn.disabled=false;refresh()},300)}
$('#submit').onclick=async()=>{const kw=$('#keywords').value.split(',').map(x=>x.trim()).filter(Boolean).slice(0,3);const q=$('#question').value.trim();const fb=$('#feedback').value.trim();if(!kw.length&&!q&&!fb){$('#msg').textContent='키워드 또는 질문을 입력해주세요.';$('#msg').className='err';return}
$('#submit').disabled=true;$('#msg').textContent='반영 중...';$('#msg').className='help';try{if(kw.length){const rows=[...new Set(kw)].map(word=>({word,device_id:did}));const {error}=await sb.from('eum_keywords').insert(rows);if(error&&error.code!=='23505')throw error}
if(q){const author=$('#named').checked?($('#author').value.trim()||'참가자'):'익명';const {error}=await sb.from('eum_questions').insert({body:q,author,device_id:did});if(error)throw error}
